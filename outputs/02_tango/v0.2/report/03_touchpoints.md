...
geometry-driven UBEM assumptions" [[doi:10.2139/ssrn.7333555]]. It entered the core tier in
v0.2; in v0.1 the source could not be read at all.
...
[Certain] In v0.1 two further rows carried `none` for a different reason again:
`doi:10.1039/d5dd00435g` and `doi:10.2139/ssrn.7333555` could not be read by any route, so no
touchpoint could be evidenced for either. Both were read from the local PDF store in v0.2 and
