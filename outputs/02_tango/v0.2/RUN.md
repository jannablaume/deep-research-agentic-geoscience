outputs/02_tango/v0.2

## Run log

- date: 2026-09-11 (v0.1), 2026-09-11 (v0.2 re-read)
...
**v0.2 is v0.1 plus a hand-retrieval round. No re-harvest, no re-triage, no re-screening.**
`screened.csv`, `triage.csv`, `queries.csv`, `shortlist.md`, `audit_sample.md` and
`triage_stats.md` are byte-identical to v0.1, and the admitted set is the same 183 sources. What
changed is what could be *read*: fourteen sources were retrieved by hand into a local PDF store
outside this repository and read, three of them promoting back to `core`. v0.1 remains on disk
and is the record of what the run measured without them. Step 7 below records the round in
full; every other step in this file describes v0.1 and is unchanged except where a count moved.
...
intended deep-read — reduced to 11% in v0.2, see step 7 — and it happened *after* the step-3a handoff rather than before it, which is
...
Two of the fifteen were empty rather than thin. `doi:10.1039/d5dd00435g` yielded no narrative
text by any route — publisher, reader proxy, ScienceDirect mirror, OpenAlex, Europe PMC, OSTI
and arXiv were all tried — so its `tango_touchpoints` was `none` and every other field
`not stated`. `doi:10.2139/ssrn.7333555` had no abstract in any harvested API record either, so
nothing beyond its title existed in v0.1. **`none` in those two rows meant "no touchpoint
could be evidenced", not "this system has no touchpoint"** — and v0.2 is the proof, because
both were read from hand-retrieved PDFs and both now carry six touchpoints. Section 03 records
the correction.
...
`python3 scripts/audit_tango.py --out outputs/02_tango/v0.2` returns **38/38 passed.**
...
| **Admitted** | **183 — 101 `core`, 82 `context`** (v0.1: 98 / 85) |
...
**Access, after v0.2.** 12 core records still demoted for want of full text (11% of the
intended deep-read, down from 15 and 13%); 0 admitted sources unreadable by any route (was 2);
2 core records still read from an inferred-identity substitute (was 8); 1 still read through a
reader proxy with table values unquotable (was 4); 1 still read from a third-party deposit
(was 3); 15 sources read from the local PDF store across the two versions.
...
## Step 7 — The v0.2 hand-retrieval round

### What was asked for and what arrived

After v0.1 closed, the person running it worked through `unreachable.md` — not `paywalled.md` —
and retrieved the sources it named, placing 20 PDFs in a local store outside this repository.
That distinction matters and is worth recording, because `unreachable.md` is the wider file: it
covers sources that *were* read but not from the place their identity key points at, and that
is where most of this round's value turned out to be.
...
|---|---|---|
| blocked in v0.1, `context` at `abstract-only` | 3 | promotion to `core`; two of the three were empty rows |
...
| already read in full from the same route in v0.1 | 3 | nothing — confirmed and skipped |
...
(read from the local store in v0.1 already), `doi:10.69997/pse.120458` (a two-page abstract read
whole in v0.1) and `2605.19743v2.pdf`, which is the arXiv substitute for
`doi:10.3929/ethz-c-000801434` rather than the institutional version — so it could not verify
anything v0.1 had not already used.

### The two discarded files, both predicted by v0.1
...
- `doi:10.1145/3731599.3767349` — the ACM version of record describes **two** Parsl
  implementations where the preprint described one, drops LangGraph from the sentence v0.1
  quoted, and adds two things the preprint does not have: a hallucination incident in which the
  agent invented three PDB IDs and molecular dynamics ran on the wrong protein structures, and a
  hard tool-call cap "around 24" that forced the second implementation. `failure_handling` was
  `not stated` in v0.1; it now records three observed, unhandled failures.
- `doi:10.1016/j.dche.2026.100312` — the published paper has **three** case studies, not two.
  ...
  synthesis flowsheet. v0.1's `evaluation_method` was therefore incomplete.
- `doi:10.1016/j.taml.2026.100660` — the published version reports a pass@1 comparison (86.6%
  against 85.0% on paraphrased prompts) that v0.1 explicitly recorded as absent, writing that
  the evaluation was "focused on result presentation rather than quantitative executability
...
as a measurement about the source, not an accusation, and it was simply unreadable in v0.1.
...
- `doi:10.26434/chemrxiv.15006587/v1` → `core`, `M2`. No new system: it shares `system_id`
  `Aspen-MCP` with the repository record, so per-system counts are unchanged. v0.1's
  `would_change` column predicted exactly this — "little, uniquely" — and was right.

**No source entered or left the admitted set.** The screening decisions, the shortlist, the
audit sample and the false-negative rate are all v0.1's, untouched.
...
untouched and identical to v0.1.
...
touchpoint against its own block, which now reports 0 unevidenced touchpoints across all 98
core blocks that declare any. `audit_tango.py` does not check this, which is why it was worth
checking by hand.

### What v0.2 does not settle

Twelve demotions remain and `paywalled.md` still records what each would change. Four of them
are ChemRxiv and two SSRN — the same two hosts that caused most of v0.1's losses, and neither
was resolved. Two core records are still read from a substitute whose identity is inferred:
`doi:10.3929/ethz-c-000801434`, where the hand-retrieved copy is the same arXiv version and so
verifies nothing, and `title:aspenplusmcp…`, still asserted by the repository itself — though
that system's evidence no longer rests on the repository alone now that its preprint is read.
One record is still proxy-read with tables unquotable (`doi:10.20944/preprints202608.1323.v1`)
and one still read from a laboratory deposit (`doi:10.11578/dc.20260516.1`).

The divergence rate is the finding most likely to matter for the next run: **three of six
