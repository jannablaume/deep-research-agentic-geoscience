...
**v0.2 changed this file more than any other.** After v0.1 was written, fourteen of the
sources recorded below were retrieved by hand from publisher and preprint sites and placed in
a local store outside this repository. All fourteen were read. What that recovered, and what
it did not, is recorded in each section and summarised under "What the v0.2 re-read settled".

## Screened core, demoted to context for want of full text

Fifteen records screened `core` could not be read in full when the v0.1 deep-read reached
them, and one further record was blocked at the step-3a handoff and stayed blocked. **Three of
those fifteen were recovered in v0.2 and promoted back to `core`**; the remaining twelve are
...
98 in v0.1, recovered to 101 in v0.2, so 11% of the intended deep-read remains lost to
access** — down from 13%.

Two of the fifteen were worse than thin — they were empty. Both are now read:

| identity_key | title | what v0.1 had | what v0.2 has |
|---|---|---|---|
...
Both carried `tango_touchpoints: none` in v0.1. Both now carry six touchpoints each. This is
the concrete case for the warning v0.1 wrote and v0.2 can now confirm: **`none` produced by an
access failure looked identical in the column to a genuine negative, and in both cases the
...
Eight `core` records were read in v0.1 from a substitute that was almost certainly the same
work but was not verified against the version of record. **Six of the eight were verified in
v0.2 by reading the version of record from the local store.** Three of those six turned out to
differ from their preprint in ways that changed this run's records — which is the concrete
answer to the risk v0.1 could only state.

| identity_key | what v0.1 read instead | why | v0.2 outcome |
|---|---|---|---|
...
| `doi:10.1016/j.taml.2026.100660` | arXiv:2503.01273v1 | Elsevier is an established blocked host | **verified, and corrected** — the published version reports a pass@1 robustness comparison (86.6% against 85.0% on paraphrased prompts) that v0.1 recorded as absent |
...
Two things are worth stating plainly. First, the concentration v0.1 identified holds: seven of
the eight are publisher blocks, six of those Elsevier or Wiley, and in every case an
...
a small sample.** v0.1 flagged this as a risk it could not size; on these six it is not
negligible.

## Admitted and read, but through a proxy or a third-party deposit

Seven `core` records were reached in v0.1 by a route other than the publisher's own site.
**Five were re-read from the publisher's own PDF in v0.2**; two were not.

- **Through the `r.jina.ai` reader proxy** (4 in v0.1). Three are now read from the publisher
  PDF: `doi:10.1109/access.2025.3605803`, `doi:10.1080/19401493.2026.2653969`,
  `doi:10.3390/buildings15173190`. One remains proxy-read:
  `doi:10.20944/preprints202608.1323.v1`. The cost v0.1 recorded was specific — the proxy
  renders tables and figures as captions and alternative text, so **table cell values were not
  quotable** — and lifting it changed a finding. In `doi:10.3390/buildings15173190` the
  recovered Table 9 shows GPT+MCP max-displacement errors of 1.624%, 1.998% and 2.834% against
  an abstract claiming "deviations under 1.5%", so three of eight case-direction pairs exceed
  the paper's own headline figure. That discrepancy was unreadable in v0.1 and is recorded in
  `papers.md` and section 08.
- **From a laboratory or repository deposit** (3 in v0.1). Two are now read from the Elsevier
  version of record: `doi:10.1016/j.softx.2025.102367` and
...
`doi:10.69997/pse.120458` is not an access failure. It was retrieved and read completely in
v0.1, and the complete record is a two-page conference extended abstract of roughly 450 words
with no methods, evaluation or limitations sections in existence. Its `papers.csv` row is
sparse because the source is sparse. A copy of it was among the fourteen hand-retrieved PDFs
...
## One source was recovered from the local store in v0.1
...
- `title:automaticbuildingenergymodel…` is indexed as a report; it is a peer-reviewed journal
  ...
  `peer-reviewed` on the evidence of the document, not the index. v0.2 read the publisher
  version and confirms the volume, year and article number.
...
- `doi:10.3390/buildings15173190` carried `2507.1968` in the arXiv field, which is a
  ...
  record would have been fabricated wholesale. **This warning was tested in v0.2**: the
  hand-retrieval round included `2507.01968v1.pdf`, which is that unrelated
...
| already read in full from the same route in v0.1, no change | 3 |
...
Twelve demotions remain, listed in `paywalled.md`. Nothing in this run resolved them, and the
